...
#if USE_KHEAP
struct userPageInfoElement {
    LIST_ENTRY(userPageInfoElement) prev_next_info;
    uint32 v_address;
    uint32 size;
    uint8 marked;
    int32 envid;
};
LIST_HEAD(userPageInfoElement_List, userPageInfoElement);
struct userPageInfoElement_List userFreePagesList ;
struct userPageInfoElement_List userMarkedPagesList ;
#endif
