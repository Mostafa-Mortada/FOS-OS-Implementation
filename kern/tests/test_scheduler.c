	...
	if (firstTimeTest)
	{
		...
		for (int i = 0; i < INSTANCES_NUMBER/2; i++)
		{
			...
			if (i == 4)
			{
				...
				cprintf("Creating small fib env with env id %d and priority %d \n", env->env_id, env->priority);
			}
			else
			{
			...
			cprintf("Setting priority for env with ID %d to priority %d\n", env->env_id, priority_values[priority_index]);

			            env_set_priority(env->env_id, priority_values[priority_index]);

			            cprintf("Created env with ID %d and priority %d\n", env->env_id, env->priority);
			if (env == NULL)
...
void test_priorityRR_1()
{
	...
	if (firstTimeTest)
	{
		...
		cprintf("Setting priority for env with ID %d to priority %d\n", fibPri2ParentEnv->env_id, 4);

		env_set_priority(fibPri2ParentEnv->env_id, 4);

		cprintf("Updated env with ID %d and priority %d\n", fibPri2ParentEnv->env_id, fibPri2ParentEnv->priority);
