...
void sched_delete_ready_queues()
{
	...
	acquire_kspinlock(&ProcessQueues.qlock);
	{
		if (ProcessQueues.env_ready_queues != NULL){
			...
			ProcessQueues.env_ready_queues = NULL;
		}
		if (quantums != NULL){
			kfree(quantums);
			quantums = NULL;
...
void sched_insert_ready(struct Env* env)
{
	...
	assert(env != NULL);
	{
		//cprintf("\nInserting %d into ready queue 0\n", env->env_id);
		env->env_status = ENV_READY ;
		env->start_time = ticks;
		if(env->priority >= num_of_ready_queues){
...
void env_set_priority(int envID, int priority)
{
	...
	if(en->env_status == ENV_READY){
		sched_remove_ready(en);
		sched_insert_ready(en);
	}
