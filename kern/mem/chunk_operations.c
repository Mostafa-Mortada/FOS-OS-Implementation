...
//		return;
	...
	for(int i = 0 ; i < num_pages; i++ ){
		...
		int ret = get_page_table(page_dir ,virtual_address + i*PAGE_SIZE ,&ptr_page_table);
		if (ret == TABLE_NOT_EXIST || ptr_page_table == NULL) {
			ptr_page_table= create_page_table(page_dir,virtual_address + i*PAGE_SIZE) ;
		}
		ptr_page_table [PTX(virtual_address + i*PAGE_SIZE)] = ptr_page_table [PTX(virtual_address + i*PAGE_SIZE)] | (PERM_UHPAGE ) ;
